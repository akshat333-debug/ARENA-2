...
| **M10** ✅ | `llm/` | Cached Ollama client; **LLM attack planner** (`attacker.py`) + payload text; held-out transfer sweep | cache hit/miss correct; degrades to the scripted chain if Ollama is absent or a plan fails validity; **plan validity is ground truth** (taint-tracker replay must complete the objective), not keywords; **the two sweep arms provably differ** — the audit found them byte-identical (0/183 decisions), now 99/230. Result: causal monitor TPR@5%FPR 0.875 → 0.675 on LLM-planned attacks. | M8, M9 |
