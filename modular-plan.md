...
| **M2** | `taint.py`, `rewards.py` | Ground-truth data-flow tracker; asymmetric reward engine | taint propagates sensitive→transform→sink; the canonical `.env → summarize → webhook` chain is detected as completed exfiltration; benign chains are not; reward signs behave at every branch | M1 |
